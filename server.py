...
def sync_subtitles_for_video(video_path: str):
    if not os.path.exists(video_path):
        print(f"[WARN] File path does not exist inside container: {video_path}")
        return

    base_name = os.path.splitext(video_path)[0]
    # Glob for any subtitle variant: .ara.srt, .ara.0.srt, etc.
    srt_candidates = glob.glob(f"{glob.escape(base_name)}*.srt")

    if not srt_candidates:
        print(f"[INFO] No .srt files found alongside {video_path}")
        return
...
async def jellyfin_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    if video_path:
        print(f"[EVENT] Received webhook for: {video_path}")
